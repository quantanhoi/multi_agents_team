...
export function SettingsPage() {
  ...
  const mutation = useMutation({
    mutationFn: () => api.settings.update({ working_dir: workingDir, ollama_endpoint: ollamaEndpoint, default_temperature: temperature }),
    onSuccess: () => {
  ...
  return (
    <div className="max-w-lg">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div><label className="block text-sm font-medium mb-1">Ollama API Endpoint</label><input className="border rounded w-full px-3 py-2 text-sm" value={ollamaEndpoint} onChange={e => setOllamaEndpoint(e.target.value)} /></div>
